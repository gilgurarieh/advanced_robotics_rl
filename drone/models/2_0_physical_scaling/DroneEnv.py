...
class DroneEnv(gymnasium.Env):
    def __init__(self, reward_type):
        ...
        self.action_scaling = (0.12+0.1*4)*2.5*9.81/4
        self.random_square = 0.3
    ...
    def calculate_reward(self, terminated, termination_reason):
        ...
        r_stabilize = 2 -abs(alpha)/np.pi -abs(beta)/np.pi -linear_velocity/10 -angular_velocity/10
        r_vertical = 2 -abs(height-target_height) / 0.3  # normalize by maximum dist
        drone_pos = state_dict["drone_pos"]
        target_pos = state_dict["target_pos"]
        r_position = 2 -np.sqrt((drone_pos[0]-target_pos[0])**2 + (drone_pos[1]-target_pos[1])**2)

        if self.reward_type == "stabilize":
            reward = r_stabilize
            if terminated and termination_reason in ["reached_ground"]:
                reward -= 100
            elif terminated:
                reward -= 10
            ...
            if terminated and termination_reason in ["vertical"]:
                reward -= 100
            elif terminated:
                reward -= 10
            ...
            if terminated and termination_reason in ["vertical", "horizontal"]:
                reward -= 100
            elif terminated:
                reward -= 10
