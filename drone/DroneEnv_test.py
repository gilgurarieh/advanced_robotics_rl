...
class DroneEnv(gymnasium.Env):
    ...
    def calculate_reward(self, terminated, termination_reason):
        ...
        # Penalize linear and angular velocities
        linear_velocity_penalty = np.linalg.norm(state_dict["drone_v_lin"])
        angular_velocity_penalty = np.linalg.norm(state_dict["drone_v_ang"])

        reward = (reward_scaling * max([0, 1 - abs(height - target_height)])
                  - abs(alpha) - abs(beta)
                  - linear_velocity_penalty
                  - 2 * angular_velocity_penalty)  # More penalty for angular velocity

        if terminated:
            if termination_reason == "angle_too_big":
                reward -= 100  # High penalty for termination due to large angle
            else:
                reward -= 50  # Regular penalty for other termination reasons
    ...
    def check_terminal_state(self):
        ...
        if horizontal_distance > 0.5:
            ...
            return True, "too_far_from_target - horizontal"
        if vertical_distance > 0.2:
            print("Terminated: too far from target")
            return True, "too_far_from_target - vertical "
